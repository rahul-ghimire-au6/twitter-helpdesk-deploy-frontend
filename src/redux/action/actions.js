...
export const fetch_user_data= (token)=> async dispatch=>{
    let fetcheddata = await fetch('https://twitter-159.herokuapp.com/fetch_user_data',{headers: {'Authorization':token}})
...
export const fetch_user_tweets_from_database=(token)=>dispatch=>{
    fetch('https://twitter-159.herokuapp.com/fetch_user_tweets_from_database',{
            headers: {
                'Authorization':token
            }
        })
